...
from pyscf1.pyscf.mcscf import custom_casci as casci
...
WITH_MICRO_SCHEDULER = getattr(
    __config__, "mcscf_mc1step_CASSCF_with_micro_scheduler", False
)
WITH_STEPSIZE_SCHEDULER = getattr(
    __config__, "mcscf_mc1step_CASSCF_with_stepsize_scheduler", True
)
...
def gen_g_hop(casscf, mo, u, casdm1, casdm2, eris):
    ...
    for i in range(nmo):
        ...
        vhf_a[i] = (
            numpy.einsum("quv,uv->q", jbuf, casdm1)
            - numpy.einsum("uqv,uv->q", kbuf, casdm1) * 0.5
        )
    ...
    def gorb_update(u, fcivec):
        ...
        vhf_c = reduce(numpy.dot, (mo1.T, vj[0] - vk[0] * 0.5, mo1[:, :nocc]))
        vhf_a = reduce(numpy.dot, (mo1.T, vj[1] - vk[1] * 0.5, mo1[:, :nocc]))
...
def rotate_orb_cc(
    ...
    u = 1
    g_orb, gorb_update, h_op, h_diag = casscf.gen_g_hop(
        mo, u, fcasdm1(), fcasdm2(), eris
    )
    g_kf = g_orb
    ...
    for ah_end, ihop, w, dxi, hdxi, residual, seig in ciah.davidson_cc(
        ...
        if (
            ah_end
            or ihop == casscf.ah_max_cycle
            or (  # make sure to use the last step
                (norm_residual < casscf.ah_start_tol)
                and (ihop >= casscf.ah_start_cycle)
            )
            or (seig < casscf.ah_lindep)
        ):
            imic += 1
            ...
            if ikf > 1 and norm_gorb > norm_gkf * casscf.ah_grad_trust_region:
                ...
                ikf >= max(casscf.kf_interval, -numpy.log(norm_dr + 1e-7))
                or
                ...
                log.debug(
                    "    |g|=%5.3g (keyframe), |g-correction|=%5.3g", norm_gkf1, norm_dg
                )
                #
                # Special treatment if out of trust region
                #
                if (
                    norm_dg > norm_gorb * casscf.ah_grad_trust_region
                    and norm_gkf1 > norm_gkf
                    and norm_gkf1 > norm_gkf * casscf.ah_grad_trust_region
                ):
                    log.debug(
...
def kernel(
    ...
    while not conv and imacro < casscf.max_cycle_macro:
        ...
        for u, g_orb, njk, r0 in rota:
            ...
            if imicro >= max_cycle_micro:
                log.debug(
                    "micro %d  |u-1|=%5.3g  |g[o]|=%5.3g", imicro, norm_t, norm_gorb
                )
                break

            fcivec_a = copy.deepcopy(fcivec)
            gci = casscf.update_casdm(
                mo, u, fcivec_a, e_cas, eris, locals(), initial_state=circ
            )
            ...
            if isinstance(gci, numpy.ndarray):
                ...
                log.debug(
                    "micro %d  |u-1|=%5.3g  |g[o]|=%5.3g  |g[c]|=%5.3g ",
                    imicro,
                    norm_t,
                    norm_gorb,
                    norm_gci,
                ...
                norm_gci = None
                log.debug(
                    "micro %d  |u-1|=%5.3g  |g[o]|=%5.3g  |g[c]|=%s ",
                    imicro,
                    norm_t,
                    norm_gorb,
                    norm_gci,
            ...
            if norm_t < conv_tol_grad or (norm_gorb < conv_tol_grad * 0.5):
                break
        ...
        if (
            abs(de) < tol
            and norm_gorb0 < conv_tol_grad
            and (max_offdiag_u < casscf.small_rot_tol or casscf.small_rot_tol == 0)
        ):
            conv = True
    ...
    if conv:
        log.info(
            "1-step CASSCF converged in %d macro (%d JK %d micro) steps",
            imacro,
            totinner,
            totmicro,
        )
        ...
        log.info(
            "1-step CASSCF not converged, %d macro (%d JK %d micro) steps",
            imacro,
            totinner,
            totmicro,
        )
    ...
    if casscf.canonicalization:
        ...
        mo, fcivec, mo_energy = casscf.canonicalize(
            mo, fcivec, eris, casscf.sorting_mo_energy, casscf.natorb, casdm1, log
        )
...
class CASSCF(casci.CASCI):
    ...
    max_stepsize = getattr(__config__, "mcscf_mc1step_CASSCF_max_stepsize", 0.02)
    ...
    ah_grad_trust_region = getattr(
        __config__, "mcscf_mc1step_CASSCF_ah_grad_trust_region", 3.0
    )

    internal_rotation = getattr(
        __config__, "mcscf_mc1step_CASSCF_internal_rotation", False
    )
    ci_response_space = getattr(__config__, "mcscf_mc1step_CASSCF_ci_response_space", 4)
    ci_grad_trust_region = getattr(
        __config__, "mcscf_mc1step_CASSCF_ci_grad_trust_region", 3.0
    )
    ...
    canonicalization = getattr(
        __config__, "mcscf_mc1step_CASSCF_canonicalization", True
    )
    sorting_mo_energy = getattr(
        __config__, "mcscf_mc1step_CASSCF_sorting_mo_energy", False
    )
    scale_restoration = getattr(
        __config__, "mcscf_mc1step_CASSCF_scale_restoration", 0.5
    )
    ...
    def dump_flags(self, verbose=None):
        ...
        log.info(
            "augmented hessian ah_grad_trust_region = %g", self.ah_grad_trust_region
        )
        ...
        if getattr(self._scf, "with_solvent", None) and not getattr(
            ...
            log.warn(
                """Solvent model %s was found at SCF level but not applied to the CASSCF object.
...
""",
    ...
    def casci(
        ...
        e_tot, e_cas, fcivec, circ = casci.kernel(
            fcasci, mo_coeff, ci0, log, envs=envs, initial_state=initial_state
        )
        ...
        if envs is not None and log.verbose >= logger.INFO:
            ...
            if "imicro" in envs:  # Within CASSCF iteration
                ...
                if "norm_gci" in envs and envs["norm_gci"] is not None:
                    ...
                    log.info(
                        "               |grad[o]|=%5.3g  |ddm|=%5.3g  |maxRot[o]|=%5.3g",
                        envs["norm_gorb0"],
                        envs["norm_ddm"],
                        envs["max_offdiag_u"],
                    )
    ...
    def update_ao2mo(self, mo):
        raise DeprecationWarning(
            "update_ao2mo was obseleted since pyscf v1.0.  " "Use .ao2mo method instead"
        )
    ...
    def ao2mo(self, mo_coeff=None):
        ...
        return mc_ao2mo._ERIS(self, mo_coeff, method="incore", level=self.ao2mo_level)
    ...
    def update_casdm(self, mo, u, fcivec, e_cas, eris, envs={}, initial_state=None):
        nmo = mo.shape[1]
        rmat = u - numpy.eye(nmo)
        ...
        if self.with_dep4:
            ...
            h1 = reduce(numpy.dot, (ua.T, h1e_mo, ua)) + reduce(
                numpy.dot, (mo1_cas.T, vj - vk * 0.5, mo1_cas)
            )
            ...
            for i in range(nmo):
                ...
                kbuf = eris.papa[i]
                jk += (
                    numpy.einsum("quv,q->uv", jbuf, ddm[i])
                    - numpy.einsum("uqv,q->uv", kbuf, ddm[i]) * 0.5
                )
                p1aa[i] = lib.dot(ua.T, jbuf.reshape(nmo, -1))
                paa1[i] = lib.dot(kbuf.transpose(0, 2, 1).reshape(-1, nmo), ra)
        ...
        ecore = numpy.einsum("pq,pq->", h1e_mo, ddm) + numpy.einsum(
            "pq,pq->", eris.vhf_c, ddm
        )
        ### hessian_co part end ###

        g = self.solve_approx_ci(
            h1,
            h2,
            fcivec,
            ecore,
    ...
    def solve_approx_ci(self, h1, h2, ci0, ecore, e_cas, envs, initial_state=None):
        """Solve CI eigenvalue/response problem approximately"""
        ncas = self.ncas
        nelecas = self.nelecas
        if "norm_gorb" in envs:
            tol = max(self.conv_tol, envs["norm_gorb"] ** 2 * 0.1)
        else:
            tol = None
        if getattr(self.fcisolver, "approx_kernel", None):
            ...
            getattr(self.fcisolver, "contract_2e", None)
            and getattr(self.fcisolver, "absorb_h1e", None)
        ):
    ...
    def get_grad(self, mo_coeff=None, casdm1_casdm2=None, eris=None):
        ...
        if casdm1_casdm2 is None:
            e_tot, e_cas, civec, circ = self.casci(mo_coeff, self.ci, eris)
    ...
    def rotate_mo(self, mo, u, log=None):
        ...
        if log is not None and log.verbose >= logger.DEBUG:
            ...
            log.debug(
                "Active space overlap to initial guess, SVD = %s",
                numpy.linalg.svd(s)[1],
            )
            log.debug(
                "Active space overlap to last step, SVD = %s",
    ...
    def max_stepsize_scheduler(self, envs):
        ...
        if envs["de"] > -self.conv_tol:  # Avoid total energy increasing
            self._max_stepsize *= 0.3
            logger.debug(self, "set max_stepsize to %g", self._max_stepsize)
        else:
            self._max_stepsize = (self.max_stepsize * self._max_stepsize) ** 0.5
    ...
    def max_orb_stepsize(self, x):  # pragma: no cover
        sys.stderr.write(
            'WARN: Attribute "max_orb_stepsize" was replaced by "max_stepsize"\n'
        )
        self.max_stepsize = x
    ...
    def ci_update_dep(self, x):  # pragma: no cover
        sys.stderr.write(
            "WARN: Attribute .ci_update_dep was replaced by .with_dep4 since PySCF v1.1.\n"
        )
        self.with_dep4 = x == 4
